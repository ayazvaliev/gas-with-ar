...
class SyntDataLoaders:
    ...
    Attributes:
        ...
        test_loader (DataLoader): Dataloader with test data subset.
            Contains last `config.validation_size` items from the whole dataset (teacher pickle file).
        vis_batch (tuple): The first batch of the train subset for logging visualization purposes.
    """

    def __init__(self, config: ConfigDict, seed: int = 42):
        self.config = config
        ...
        worker_init_fn = (
            lambda wid: _worker_init_fn(wid, seed)
